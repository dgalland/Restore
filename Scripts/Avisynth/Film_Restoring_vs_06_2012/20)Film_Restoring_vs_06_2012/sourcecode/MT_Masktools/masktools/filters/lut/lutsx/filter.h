...
typedef void(Processor)(Byte *pDst, int nDstPitch, const Byte *pSrc1, int nSrc1Pitch, const Byte *pSrc2, int nSrc2Pitch, const Byte *lut, const int *pCoordinates, int nCoordinates, int nWidth, int nHeight);
...
protected:
   ...
   {
      processors.best_processor( constraints[nPlane] )( dst, dst.pitch(), frames[0].plane(nPlane), frames[0].plane(nPlane).pitch(), frames[1].plane(nPlane), frames[1].plane(nPlane).pitch(), luts[nPlane], pCoordinates, nCoordinates, dst.width(), dst.height() );
   }
...
public:
   Filter(const Parameters &parameters) : MaskTools::Filter<InPlaceFilter>( parameters )
   {
      ...
      /* choose the mode */
      processors.push_back( processors_array[ ModeToInt( parameters["mode"].toString() ) ][ ModeToInt( parameters["mode2"].toString() ) ] );
   }
