...
typedef void(Processor)(Byte *pDst, int nDstPitch, const Byte *pSrc1, int nSrc1Pitch,
                        const Byte *pSrc2, int nSrc2Pitch, int nWidth, int nHeight);

Processor merge_c;
Processor merge_luma_420_c;

extern "C" Processor Merge_merge4_mmx;
...
public:
   Filter(const Parameters &parameters) : MaskTools::Filter<InPlaceFilter>( parameters )
   {
      ...
      processors.push_back( Filtering::Processor<Processor>( Merge_merge4_mmx, Constraint( CPU_MMX, 4, 1, 1, 1 ), 1 ) );
