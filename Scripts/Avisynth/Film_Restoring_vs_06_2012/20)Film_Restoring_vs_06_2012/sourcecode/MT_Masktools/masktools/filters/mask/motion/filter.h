...
typedef bool (Processor)(Byte *pDst, int nDstPitch, const Byte *pSrc, int nSrcPitch, int nLowThreshold, int nHighThreshold, int nMotionThreshold, int nSceneChange, int nSceneChangeValue, int nWidth, int nHeight);

extern Processor *mask_c;
extern Processor *mask8_mmx;
extern Processor *mask8_isse;
extern Processor *mask8_3dnow;
...
public:
   Filter(const Parameters &parameters) : MaskTools::Filter<InPlaceFilter>( parameters )
   {
      ...
      processors.push_back(Filtering::Processor<Processor>(mask8_3dnow, Constraint(CPU_3DNOW, 8, 1, 1, 1), 3));
   }
