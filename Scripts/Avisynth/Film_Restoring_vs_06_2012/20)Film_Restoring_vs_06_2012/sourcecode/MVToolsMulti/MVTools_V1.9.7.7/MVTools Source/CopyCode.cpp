...
void PlaneCopy(unsigned char* dstp, int dst_pitch, const unsigned char* srcp, int src_pitch, int row_size, int height,
               bool isse)
{
    ...
    if (isse) {
        if ((height == 1) || (src_pitch == dst_pitch && dst_pitch == row_size)) {
            ...
            asm_PlaneCopy_ISSE(dstp, dst_pitch, srcp, src_pitch, row_size, height);
        }
        return;
    }
    else {
...
void MemZoneSet(unsigned char *ptr, unsigned char value, int width, int height, int offsetX, int offsetY, int pitch)
{
