...
{
    if (fGamma == 1.0)
        *occMask = static_cast<unsigned char>(
                   std::max<int>(*occMask, std::min<int>(static_cast<int>(255 * occlusion*occnorm), 255)));
    else
        *occMask= static_cast<unsigned char>(
                  std::max<int>(*occMask, std::min<int>(static_cast<int>(255 * pow(occlusion*occnorm, fGamma)), 255)));
}

void MakeVectorOcclusionMaskTime(MVClip &mvClip, int nBlkX, int nBlkY, double dMaskNormFactor, double fGamma, 
                                 int nPel, BYTE * occMask, int occMaskPitch, int time256, int blkSizeX, int blkSizeY)
...
void VectorMasksToOcclusionMaskTime(BYTE *VXMask, BYTE *VYMask, int nBlkX, int nBlkY, double dMaskNormFactor, 
                                    double fGamma, int nPel, BYTE * occMask, int occMaskPitch, int time256, 
                                    int blkSizeX, int blkSizeY)
{ // analyse vectors field to detect occlusion
    // note: dMaskNormFactor = 1/ml
    MemZoneSet(occMask, 0, nBlkX, nBlkY, 0, 0, nBlkX);
    int time4096X = time256*16/blkSizeX;
...
void MakeVectorOcclusionMask(MVClip &mvClip, int nBlkX, int nBlkY, double dMaskNormFactor, double fGamma, int nPel,
                             BYTE * occMask, int occMaskPitch)
{ // analyse vectors field to detect occlusion
    // note: dMaskNormFactor = 1/ml
    ...
    for (int by=0; by<nBlkY; by++) {
        for (int bx=0; bx<nBlkX; bx++) {
            ...
            if (fGamma == 1.0)
                occMask[bx + by*occMaskPitch] = static_cast<int>(std::min<int>(static_cast<int>(255 * occlusion*occnorm),255));
            else
                occMask[bx + by*occMaskPitch]= static_cast<unsigned char>(std::min<int>(static_cast<int>(255 * pow(occlusion*occnorm, fGamma)), 255));
        }
    }
}

// it is olr pre 1.8 mask
void VectorMasksToOcclusionMask(BYTE *VXMask, BYTE *VYMask, int nBlkX, int nBlkY, double dMaskNormFactor, double fGamma,
                                int nPel, BYTE * occMask)
{ // analyse vectors field to detect occlusion
    // note: dMaskNormFactor = 1/ml
    double occnorm = 10* dMaskNormFactor/nPel; // empirical
    for (int by=0; by<nBlkY; by++) {
        for (int bx=0; bx<nBlkX; bx++) {
            ...
            if (fGamma == 1.0)
                occMask[i] = static_cast<int>(std::min<int>(static_cast<int>(255 * occlusion*occnorm),255));
            else
                occMask[i]= static_cast<int>(std::min<int>(static_cast<int>(255 * pow(occlusion*occnorm, fGamma)), 255));
        }
    }
}

void MakeVectorSmallMasks(MVClip &mvClip, int nBlkX, int nBlkY, BYTE *VXSmallY, int pitchVXSmallY, BYTE*VYSmallY, 
                          int pitchVYSmallY)
{
...
unsigned char SADToMask(unsigned int sad, unsigned int sadnorm1024)
{
    ...
    unsigned int l = (sadnorm1024*sad+512)/1024;
    return (unsigned char)((l > 255) ? 255 : l);
}


// time-weihted blend src with ref frames (used for interpolation for poor motion estimation)
void Blend(BYTE * pdst, const BYTE * psrc, const BYTE * pref, int height, int width, int dst_pitch, int src_pitch,
           int ref_pitch, int time256, bool isse)
{
    // add isse
    int h, w;
    for (h=0; h<height; ++h) {
        for (w=0; w<width; ++w) {
            pdst[w] = static_cast<BYTE>((static_cast<unsigned int>(psrc[w])*(256 - time256) + 
                                         static_cast<unsigned int>(pref[w])*time256 + 128)>>8);
        }
        pdst += dst_pitch;
        psrc += src_pitch;
        pref += ref_pitch;
...
{
    for (int v=0; v<256; ++v) {
        LUTVB[v] = ((v-128)*(256-time256)+128)>>8;
        LUTVF[v] = ((v-128)*time256+128)>>8;
    }
}
    ...
    if (nPel==1)
    {
        for (int h=0; h<height; h++) {
            for (int w=0; w<width; w++) {
                ...
                pdst[w] = ( ( (dstF*(255-MaskF[w]) + 
                            ((MaskF[w]*(dstB*(255-MaskB[w])+MaskB[w]*dstF0)+255)>>8) + 255)>>8 )*(256-time256) +
                            ( (dstB*(255-MaskB[w]) + 
                            ((MaskB[w]*(dstF*(255-MaskF[w])+MaskF[w]*dstB0)+255)>>8) + 255)>>8 )*time256 + 128)>>8;
    ...
    if (nPel==1) {
        for (int h=0; h<height; h++) {
            for (int w=0; w<width; w+=1) {
                ...
                pdst[w] = (((Median3r(minfb,dstBB,maxfb)*MaskF[w] + dstF*(255-MaskF[w])+255)>>8)*(256-time256) +
                           ((Median3r(minfb,dstFF,maxfb)*MaskB[w] + dstB*(255-MaskB[w])+255)>>8)*time256 + 128)>>8;
                //                pdst[w] = (((Median3(dstB,dstBB,dstF0)*MaskF[w] + dstF*(255-MaskF[w])+255)>>8)*(256-time256) +
                //                           ((Median3(dstF,dstFF,dstB0)*MaskB[w] + dstB*(255-MaskB[w])+255)>>8)*time256)>>8;
                //                pdst[w]=MaskF[w];
            }
            pdst += dst_pitch;
            prefB += ref_pitch;
        ...
        for (int h=0; h<height; h++) {
            for (int w=0; w<width; w++) {
                ...
                int vxFF = LUTVF[VXFullFF[w]];
                int vyFF = LUTVF[VYFullFF[w]];
                int dstFF = prefF[vyFF*ref_pitch + vxFF + (w<<1)];
                int vxB = LUTVB[VXFullB[w]];
                int vyB = LUTVB[VYFullB[w]];
                int dstB = prefB[vyB*ref_pitch + vxB + (w<<1)];
                //				int dstB0 = prefB[(w<<1)]; // zero
                int vxBB = LUTVB[VXFullBB[w]];
                int vyBB = LUTVB[VYFullBB[w]];
                int dstBB = prefB[vyBB*ref_pitch + vxBB + (w<<1)];
        ...
        if (nPel==1) {
            for (int h=0; h<height; h++) {
                for (int w=0; w<width; w+=2) // paired for speed
                {
                    ...
                    pdst[w] = ( ((dstF + dstB)<<8) + (dstB - dstF)*(MaskF[w] - MaskB[w]) + 256)>>9;
                    pdst[w+1] = ( ((dstF1 + dstB1)<<8) + (dstB1 - dstF1)*(MaskF[w+1] - MaskB[w+1]) +256)>>9;
                }
                pdst += dst_pitch;
                prefB += ref_pitch;
                prefF += ref_pitch;
        ...
        if (nPel==1) {
            for (int h=0; h<height; h++) {
                for (int w=0; w<width; w+=2) // paired for speed
                {
                    ...
                    pdst[w] = ( ( (dstF*255 + (dstB-dstF)*MaskF[w] + 255) )*(256-time256) +
                                ( (dstB*255 - (dstB-dstF)*MaskB[w] + 255) )*time256 + 32768)>>16;
                    pdst[w+1] = ( ( (dstF1*255 + (dstB1-dstF1)*MaskF[w+1] + 255) )*(256-time256) +
                                ( (dstB1*255 - (dstB1-dstF1)*MaskB[w+1] + 255) )*time256 + 32768)>>16;
                }
                pdst += dst_pitch;
                prefB += ref_pitch;
                prefF += ref_pitch;
            ...
            for (int h=0; h<height; h++) {
                for (int w=0; w<width; w+=1) {
                    ...
                    int dstB = prefB[vyB*ref_pitch + vxB + (w<<1)];
                    pdst[w] = ( ( (dstF*(255-MaskF[w]) + dstB*MaskF[w] + 255)>>8 )*(256-time256) +
                                ( (dstB*(255-MaskB[w]) + dstF*MaskB[w] + 255)>>8 )*time256 + 128)>>8;
                }
                pdst += dst_pitch;
                prefB += ref_pitch<<1;
                prefF += ref_pitch<<1;
