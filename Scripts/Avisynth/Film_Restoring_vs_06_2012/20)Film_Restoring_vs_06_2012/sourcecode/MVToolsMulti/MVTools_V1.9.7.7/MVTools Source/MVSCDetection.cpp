...
MVSCDetection::MVSCDetection(PClip _child, PClip vectors, int Ysc, int nSCD1, int nSCD2, bool mmx, bool isse, 
                             IScriptEnvironment* env) :
               GenericVideoFilter(_child),
               mvClip(vectors, nSCD1, nSCD2, env, true),
               MVFilter(vectors, "MVSCDetection", env)
{
...
PVideoFrame __stdcall MVSCDetection::GetFrame(int n, IScriptEnvironment* env)
{
    ...
    if ( mvClip.IsUsable() ) {
        if(vi.IsYV12()) {
            ...
            MemZoneSet(dst->GetWritePtr(PLANAR_U), 0, dst->GetRowSize(PLANAR_U), dst->GetHeight(PLANAR_U), 
                       0, 0, dst->GetPitch(PLANAR_U));
            MemZoneSet(dst->GetWritePtr(PLANAR_V), 0, dst->GetRowSize(PLANAR_V), dst->GetHeight(PLANAR_V), 
                       0, 0, dst->GetPitch(PLANAR_V));
        }
        else {
            MemZoneSet(dst->GetWritePtr(), 0, dst->GetRowSize(), dst->GetHeight(), 0, 0, dst->GetPitch());
        }
        ...
        if(vi.IsYV12()) {
            ...
            MemZoneSet(dst->GetWritePtr(PLANAR_U), sceneChangeValue, dst->GetRowSize(PLANAR_U), dst->GetHeight(PLANAR_U), 
                       0, 0, dst->GetPitch(PLANAR_U));
            MemZoneSet(dst->GetWritePtr(PLANAR_V), sceneChangeValue, dst->GetRowSize(PLANAR_V), dst->GetHeight(PLANAR_V), 
                       0, 0, dst->GetPitch(PLANAR_V));
        }
        else {
            MemZoneSet(dst->GetWritePtr(), sceneChangeValue, dst->GetRowSize(), dst->GetHeight(), 0, 0, dst->GetPitch());
