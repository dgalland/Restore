...
//			env->ThrowError("SimpleResize: Horizontal pixels must divide by 2");
        ...
        int AlignmentSize=128;
	    hControl = (unsigned int*) _aligned_malloc(newwidth*12+128, AlignmentSize);   // aligned for P4 cache line
	    hControlUV = (unsigned int*) _aligned_malloc(newwidth*12+128, AlignmentSize);   // added by Fizick
	    vWorkY = (unsigned int*) _aligned_malloc(2*oldwidth+128, AlignmentSize);   
	    vWorkUV = (unsigned int*) _aligned_malloc(oldwidth+128, AlignmentSize);   
	    vOffsets = (unsigned int*) _aligned_malloc(newheight*4, AlignmentSize);  
	    vWeights = (unsigned int*) _aligned_malloc(newheight*4, AlignmentSize);  
        if (!hControl || !hControlUV || !vWorkY || !vWorkUV || !vOffsets || !vWeights) {
            // no memory 
            throw "Out of memroy SimpleResize.cpp";
        }
...
{
	if (vOffsetsUV) _aligned_free (vOffsetsUV);
	if (vWeightsUV) _aligned_free (vWeightsUV);
	if (hControl)   _aligned_free (hControl);
	if (hControlUV) _aligned_free (hControlUV);
	if (vWorkY)     _aligned_free (vWorkY);
	if (vWorkUV)    _aligned_free (vWorkUV);
	if (vOffsets)   _aligned_free (vOffsets);
	if (vWeights)   _aligned_free (vWeights);
