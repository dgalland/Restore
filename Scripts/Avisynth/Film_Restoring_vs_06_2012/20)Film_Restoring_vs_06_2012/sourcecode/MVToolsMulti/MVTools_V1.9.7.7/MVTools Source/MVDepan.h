...
private:
    ...
    static int const messagebuflen=128;
    char messagebuf[messagebuflen];
...
//	FakeGroupOfPlanes *fgop;
    ...
    void RejectBadBlocks (transform tr, float blockDx[], float blockDy[], int blockSAD[], int blockX[], int blockY[], float blockWeight[], int nBlkX, int nBlkY, float neighboursDif, float globalDif, int thSCD1, float zeroWeight );

public:
    MVDepan::MVDepan(PClip _child, PClip mvs, bool _zoom, bool _rot, float _pixaspect, float _error,
                     bool _info, const char * _logfilename, float _wrong, float _zerow, int _range, 
