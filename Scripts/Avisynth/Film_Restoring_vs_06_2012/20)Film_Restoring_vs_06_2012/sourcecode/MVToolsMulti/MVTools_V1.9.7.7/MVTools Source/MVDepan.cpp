...
MVDepan::MVDepan(PClip _child, PClip mvs, bool _zoom, bool _rot, float _pixaspect, float _error, bool _info, 
                 const char * _logfilename, float _wrong, float _zerow, int _range, int nSCD1, int nSCD2,
                 bool mmx, bool isse, IScriptEnvironment* env) :
         GenericVideoFilter(_child),
         mvclip(mvs, nSCD1, nSCD2, env, true),
         MVFilter(mvs, "MVDepan", env),
         wrongDif(_wrong), zeroWeight(_zerow), range(_range), ifZoom(_zoom), ifRot(_rot), pixaspect(_pixaspect),
         error(_error), info(_info), logfilename(_logfilename)
{
    ...
    if (lstrlen(logfilename) > 0) { // v.1.2.3
        ...
        if (logfile == NULL)	env->ThrowError("MVDePan: Log file can not be created!");
    }
    else
    logfile = NULL;

    if (mvclip.nDeltaFrame != 1)
        env->ThrowError("MVDePan: motion vectors delta must be =1!");
...
MVDepan::~MVDepan()
{
    if (blockDx)     delete [] blockDx;
    if (blockDy)     delete [] blockDy;
    if (blockSAD)    delete [] blockSAD;
    if (blockX)      delete [] blockX;
    if (blockY)      delete [] blockY;
    if (blockWeight) delete [] blockWeight;
    if (logfile != NULL) fclose(logfile);

    if (motionx)    delete [] motionx;
    if (motiony)    delete [] motiony;
    if (motionzoom) delete [] motionzoom;
    if (motionrot)  delete [] motionrot;
}
        ...
        for (int i=0; i<nBlkX; i++) {
            ...
            if (i<=3 || i>= nBlkX-4 || j<=3 || j>= nBlkY-4) {
                ...
                blockWeight[n] = 0; // disable bad block with big SAD
                     ...
                     blockDx[n-1] + blockDx[n+1] + blockDx[n-1+nBlkX] + blockDx[n+nBlkX] +
                     blockDx[n+1+nBlkX])/8 - blockDx[n]) > wrongDif)
            {
                blockWeight[n] = 0; // disable blocks very different from neighbours
                     ...
                     blockDy[n-1] + blockDy[n+1] + blockDy[n-1+nBlkX] + blockDy[n+nBlkX] + 
                     blockDy[n+1+nBlkX])/8 - blockDy[n]) > wrongDif)
            {
                blockWeight[n] = 0; // disable blocks very different from neighbours
            }
...
PVideoFrame __stdcall MVDepan::GetFrame(int ndest, IScriptEnvironment* env)
{
    ...
    if (vi.IsYV12()) {
        ...
        PlaneCopy(dstp, dst_pitch, srcp, src_pitch, src_width, src_height, isse);
    ...
    int framefirst = std::max<int>(ndest - range, 0);
    int framelast = std::min<int>(ndest + range, vi.num_frames-1);
    ...
    for (int nframe=framefirst; nframe<=framelast; nframe++)
    {
        ...
        if ( nframemv >= 0 && mvclip.IsUsable() ) {
            ...
            for (; iter<5; iter++) {
                TrasformUpdate(&tr, blockDx, blockDy, blockSAD, blockX, blockY, blockWeight, nBlkX, nBlkY, safety, ifZoom0, 
                               ifRot0, &errorcur, pixaspect/nFields);
                RejectBadBlocks(tr, blockDx, blockDy, blockSAD, blockX, blockY, blockWeight, nBlkX, nBlkY, wrongDif, 
                                globalDif0, mvclip.GetThSCD1(), zeroWeight);
            }
        ...
        if (info && nframe == ndest) // type text info to output frame
        {
            ...
            sprintf_s(messagebuf, messagebuflen, "MVDepan data");
            if (vi.IsYUY2())
                DrawStringYUY2(dst,xmsg,ymsg,messagebuf);
            else
                DrawString(dst,xmsg,ymsg,messagebuf);

            sprintf_s(messagebuf, messagebuflen, "fn=%5d iter=%3d error=%7.3f", nframe, iter, errorcur);
            ymsg++;
            if (vi.IsYUY2())
                DrawStringYUY2(dst,xmsg,ymsg,messagebuf);
            else
            ...
            sprintf_s(messagebuf, messagebuflen, "     dx      dy     rot    zoom");
            ymsg++;
            if (vi.IsYUY2())
                DrawStringYUY2(dst,xmsg,ymsg,messagebuf);
            else
            ...
            sprintf_s(messagebuf, messagebuflen, "%7.2f %7.2f %7.3f %7.5f", motionx[nframe],  motiony[nframe],  motionrot[nframe],  motionzoom[nframe]);
            ymsg++;
            if (vi.IsYUY2())
                DrawStringYUY2(dst,xmsg,ymsg,messagebuf);
            else
