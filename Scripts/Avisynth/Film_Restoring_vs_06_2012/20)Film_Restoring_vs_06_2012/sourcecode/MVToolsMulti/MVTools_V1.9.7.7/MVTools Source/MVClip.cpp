...
MVClip::MVClip(const PClip &vectors, int _nSCD1, int _nSCD2, IScriptEnvironment *env, bool bCreateFakeGroupOfPlanes, int _MultiIndex) :
        GenericVideoFilter(vectors), MultiIndex(_MultiIndex)
{
    MVAnalysisData *pAnalyseFilter;
    if (MultiIndex!=-1) {
        // we fetch the handle on the analyze filter array
        ...
        pAnalyseFilter=&pAnalyseFilterArray[MultiIndex];
    }
    else {
        // we fetch the handle on the analyze filter
        pAnalyseFilter = reinterpret_cast<MVAnalysisData *>(vi.nchannels);
        if (vi.nchannels >= 0 &&  vi.nchannels < 9) // seems some normal clip instead of vectors
            env->ThrowError("MVTools: invalid vectors stream");
    }
    ...
    if (bCreateFakeGroupOfPlanes) {
        pFakeGroupOfPlanes=new FakeGroupOfPlanes(nWidth, nHeight, nBlkSizeX, nBlkSizeY, nLvCount, nPel, nOverlapX, nOverlapY, yRatioUV, nBlkX, nBlkY);
    }
    else pFakeGroupOfPlanes=0;
}

MVClip::~MVClip()
{
    if (pFakeGroupOfPlanes) delete pFakeGroupOfPlanes;
}
...
{
    //  fn = child->GetFrame(n, env);
    const int *pMv=0;
    if (MultiIndex!=-1) {
        // we fetch the correct row in the array
        const BYTE* src=fn->GetReadPtr();
        int SrcPitch=fn->GetPitch();
        pMv = reinterpret_cast<const int*>(src+SrcPitch*MultiIndex);
    }
    else {
        // we get the base pointer
        pMv = reinterpret_cast<const int*>(fn->GetReadPtr());
    }
    ...
    pFakeGroupOfPlanes->Update(pMv);// fixed a bug with lost frames
}

bool  MVClip::IsUsable(int nSCD1_, int nSCD2_) const
{
    return (!pFakeGroupOfPlanes->IsSceneChange(nSCD1_, nSCD2_)) && pFakeGroupOfPlanes->IsValid();
}
