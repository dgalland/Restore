 ...
 * $Id: //AviSynth/Main/MVTools/cpu.cpp#1 $
 *
...
uint32_t cpu_detect( void )
{
    ...
    if( !strcmp((char*)vendor, "GenuineIntel") || !strcmp((char*)vendor, "CyrixInstead") )
    {
        ...
        if( !cache )
        {
            ...
            static unsigned char const cache32_ids[] = { 0x0a, 0x0c, 0x41, 0x42, 0x43, 0x44, 0x45, 0x82, 0x83, 0x84, 0x85, 0 };
            static unsigned char const cache64_ids[] = { 0x22, 0x23, 0x25, 0x29, 0x2c, 0x46, 0x47, 0x49, 0x60, 0x66, 0x67, 0x68, 0x78, 0x79, 0x7a, 0x7b, 0x7c, 0x7c, 0x7f, 0x86, 0x87, 0 };
            uint32_t buf[4];
            int max, i=0, j;
            do {
                ...
                for(j=0; j<4; ++j)
                    if( !(buf[j]>>31) )
                        while( buf[j] )
                        {
							for(int k=0; k<sizeof(cache32_ids[0]); ++k)
								if (cache32_ids[k]==static_cast<unsigned char>(buf[j]&0xff)) {
									cache = 32;
									break;
								}
							for(int k=0; k<sizeof(cache64_ids[0]); ++k)
								if (cache64_ids[k]==static_cast<unsigned char>(buf[j]&0xff)) {
									cache = 64;
									break;
								}
	                        buf[j] >>= 8;
